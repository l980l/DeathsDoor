...
{
	m_pTalkSign->Transform()->SetRelativeScale(135.f, 38.f, 1.f);		
}
...
void CBankNPCScript::OnOverlap(CCollider3D* _Other)
{
	if (L"Player" == _Other->GetOwner()->GetName())
	{
		if (KEY_TAP(KEY::E) && LEVEL_STATE::PLAY == CLevelMgr::GetInst()->GetCurLevel()->GetState())
		{
			...
			_Other->GetOwner()->GetScript<CPlayerScript>()->SetMovable(false);

			m_pTalkSign->Transform()->SetRelativeScale(Vec3(0.f));

			//상점 Frame & Upgrade 스텟 spawn
			
			if (!CLevelMgr::GetInst()->FindObjectByName(L"BankUI"))
				CLevelSaveLoadInScript::SpawnandReturnPrefab(L"prefab\\BankUIFrame.prefab", (int)LAYER::UI, Vec3(0.f, 0.f, 100.f));
			
			if (!CLevelMgr::GetInst()->FindObjectByName(L"BankUIUpgrade"))
				CLevelSaveLoadInScript::SpawnandReturnPrefab(L"prefab\\BankUIUpgrade.prefab", (int)LAYER::UI, Vec3(0.f));
