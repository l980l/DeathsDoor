...
{
    const BVItem* a = static_cast<const BVItem*>(va);
    const BVItem* b = static_cast<const BVItem*>(vb);
...
static int createBVTree(dtNavMeshCreateParams* params, dtBVNode* nodes, int /*nnodes*/)
{
    ...
    BVItem* items       = static_cast<BVItem*>(dtAlloc(sizeof(BVItem) * params->polyCount, DT_ALLOC_TEMP));
    for (int i = 0; i < params->polyCount; i++)
    {
        ...
        {
            int   vb  = static_cast<int>(params->detailMeshes[i * 4 + 0]);
            int   ndv = static_cast<int>(params->detailMeshes[i * 4 + 1]);
            float bmin[3];
            float bmax[3];
            ...
            it.bmin[0] = static_cast<unsigned short>(dtClamp((int)((bmin[0] - params->bmin[0]) * quantFactor), 0, 0xffff));
            it.bmin[1] = static_cast<unsigned short>(dtClamp((int)((bmin[1] - params->bmin[1]) * quantFactor), 0, 0xffff));
            it.bmin[2] = static_cast<unsigned short>(dtClamp((int)((bmin[2] - params->bmin[2]) * quantFactor), 0, 0xffff));

            it.bmax[0] = static_cast<unsigned short>(dtClamp((int)((bmax[0] - params->bmin[0]) * quantFactor), 0, 0xffff));
            it.bmax[1] = static_cast<unsigned short>(dtClamp((int)((bmax[1] - params->bmin[1]) * quantFactor), 0, 0xffff));
            it.bmax[2] = static_cast<unsigned short>(dtClamp((int)((bmax[2] - params->bmin[2]) * quantFactor), 0, 0xffff));
        }
        else
        {
            ...
            // Remap y
            it.bmin[1] = static_cast<unsigned short>(dtMathFloorf((float)it.bmin[1] * params->ch / params->cs));
            it.bmax[1] = static_cast<unsigned short>(dtMathCeilf((float)it.bmax[1] * params->ch / params->cs));
        }
    }
...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
    ...
    {
        offMeshConClass = static_cast<unsigned char*>(dtAlloc(sizeof(unsigned char) * params->offMeshConCount * 2, DT_ALLOC_TEMP));
        if (!offMeshConClass)
            return false;
    ...
    unsigned char* data = static_cast<unsigned char*>(dtAlloc(sizeof(unsigned char) * dataSize, DT_ALLOC_PERM));
    if (!data)
    {
    ...
    for (int i = 0; i < params->offMeshConCount; ++i)
    {
        ...
        if (offMeshConClass[i * 2 + 0] == 0xff)
        {
            ...
            p->vertCount = 2;
            p->verts[0]  = static_cast<unsigned short>(offMeshVertsBase + n * 2 + 0);
            p->verts[1]  = static_cast<unsigned short>(offMeshVertsBase + n * 2 + 1);
    ...
    if (params->detailMeshes)
    {
        ...
        for (int i = 0; i < params->polyCount; ++i)
        {
            ...
            const int     vb  = static_cast<int>(params->detailMeshes[i * 4 + 0]);
            const int     ndv = static_cast<int>(params->detailMeshes[i * 4 + 1]);
            const int     nv  = navPolys[i].vertCount;
            dtl.vertBase      = static_cast<unsigned int>(vbase);
            dtl.vertCount     = static_cast<unsigned char>(ndv - nv);
            dtl.triBase       = static_cast<unsigned int>(params->detailMeshes[i * 4 + 2]);
            dtl.triCount      = static_cast<unsigned char>(params->detailMeshes[i * 4 + 3]);
            // Copy vertices except the first 'nv' verts which are equal to nav poly verts.
            if (ndv - nv)
            {
                memcpy(&navDVerts[vbase * 3], &params->detailVerts[(vb + nv) * 3], sizeof(float) * 3 * (ndv - nv));
                vbase += static_cast<unsigned short>(ndv - nv);
            }
        }
        // Store triangles.
    ...
    else
    {
        ...
        for (int i = 0; i < params->polyCount; ++i)
        {
            ...
            dtl.vertBase      = 0;
            dtl.vertCount     = 0;
            dtl.triBase       = static_cast<unsigned int>(tbase);
            dtl.triCount      = static_cast<unsigned char>(nv - 2);
            // Triangulate polygon (local indices).
            for (int j = 2; j < nv; ++j)
            {
                unsigned char* t = &navDTris[tbase * 4];
                t[0]             = 0;
                t[1]             = static_cast<unsigned char>(j - 1);
                t[2]             = static_cast<unsigned char>(j);
    ...
    for (int i = 0; i < params->offMeshConCount; ++i)
    {
        ...
        if (offMeshConClass[i * 2 + 0] == 0xff)
        {
            ...
            con->poly                = static_cast<unsigned short>(offMeshPolyBase + n);
