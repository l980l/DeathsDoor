                                         ...
                                         dtPolyRef*           randomRef, float* randomPt) const
{
    ...
    closestPointOnPoly(polyRef, pt, pt, nullptr);
                                                     ...
                                                     dtPolyRef*           randomRef, float*      randomPt) const
{
    ...
    closestPointOnPoly(randomPolyRef, pt, pt, nullptr);
...
public:
    ...
    virtual ~dtFindNearestPolyQuery() override;
    ...
    virtual void process(const dtMeshTile* tile, dtPoly** polys, dtPolyRef* refs, int count) override
    {
                                         ...
                                         const dtQueryFilter* filter, dtPolyQuery* query) const
{
    ...
    if (tile->bvTree)
    {
        ...
        while (node < end)
        {
            ...
            {
                dtPolyRef ref = base | static_cast<dtPolyRef>(node->i);
...
public:
    ...
    virtual ~dtCollectPolysQuery() override;
    ...
    virtual void process(const dtMeshTile* tile, dtPoly** polys, dtPolyRef* refs, int count) override
    {
        ...
        memcpy(m_polys + m_numCollected, refs, static_cast<size_t>(toCopy) * sizeof(dtPolyRef));
    ...
    *visitedCount = 0;
    ...
    while (nstack)
    {
        ...
        for (int i = 0, j = static_cast<int>(curPoly->vertCount) - 1; i < static_cast<int>(curPoly->vertCount); j = i++)
        {
            ...
            {
                const unsigned int idx = static_cast<unsigned int>(curPoly->neis[j] - 1);
                                             ...
                                             const int maxSegments) const
{
    ...
    *segmentCount = 0;
    ...
    for (int i = 0, j = static_cast<int>(poly->vertCount) - 1; i < static_cast<int>(poly->vertCount); j = i++)
    {
        ...
        else
        {
            ...
            if (poly->neis[j])
            {
                const unsigned int idx = static_cast<unsigned int>(poly->neis[j] - 1);
                                            ...
                                            float*               hitDist, float* hitPos, float* hitNormal) const
{
    ...
    while (!m_openList->empty())
    {
        ...
        // Hit test walls.
        for (int i = 0, j = static_cast<int>(bestPoly->vertCount) - 1; i < static_cast<int>(bestPoly->vertCount); j = i++)
        {
            // Skip non-solid edges.
            ...
            {
                // Internal edge
                const unsigned int idx = static_cast<unsigned int>(bestPoly->neis[j] - 1);
