...
void CCamera::SortObject()
{
    ...
    for (UINT i = 0; i < MAX_LAYER; ++i)
    {
        ...
        if (m_iLayerMask & (1 << i))
        {
            ...
            for (CGameObject* pObject : vecObject)
            {
                ...
                {
                    Vec3 vWorldPos = pObject->Transform()->GetWorldPos();
                ...
                for (UINT iMtrl = 0; iMtrl < iMtrlCount; ++iMtrl)
                {
                    ...
                    case SHADER_DOMAIN::DOMAIN_MASK:
                    {
                        ...
                        if (eDomain == SHADER_DOMAIN::DOMAIN_DEFERRED)
                            pMap = &m_mapInstGroup_Deferred;
                        else if (eDomain == SHADER_DOMAIN::DOMAIN_OPAQUE || eDomain == SHADER_DOMAIN::DOMAIN_MASK)
                            pMap = &m_mapInstGroup_Forward;
                        else
                            assert(nullptr);
                        ...
                        if (iter == pMap->end())
                            pMap->insert(make_pair(uID.llID, vector<tInstObj>{tInstObj{pObject, iMtrl}}));
                        else
                            iter->second.push_back(tInstObj{pObject, iMtrl});
                    }
                    break;
                    case SHADER_DOMAIN::DOMAIN_DECAL:
                        m_vecDecal.push_back(pObject);
                        break;
                    case SHADER_DOMAIN::DOMAIN_TRANSPARENT:
                        m_vecTransparent.push_back(pObject);
                        break;
                    case SHADER_DOMAIN::DOMAIN_POSTPROCESS:
                        m_vecPost.push_back(pObject);
                        break;
                    case SHADER_DOMAIN::DOMAIN_UI:
                        m_vecUI.push_back(pObject);
                        break;
                    }
                }
            }
...
void CCamera::clear()
{
    for (auto& pair : m_mapInstGroup_Deferred)
        pair.second.clear();
    for (auto& pair : m_mapInstGroup_Forward)
        pair.second.clear();
