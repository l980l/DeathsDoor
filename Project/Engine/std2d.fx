...
float4 PS_Flicker(VS_OUT _in) : SV_Target
{
    float4 vOutColor = (float4) 0.f;            
    ...
    float fFlashSpeed = 2.0f; // 반복 주기 (투명-불투명)의 속도 조절

    // 시간에 따른 투명도 계산
    float fAlpha = abs(sin(g_AccTime * fFlashSpeed));

    vOutColor.rgb *= fAlpha;
