        ...
        for (size_t i = 0; i < pMesh->m_vecBones.size(); ++i)
        {
            ...
            for (size_t j = 0; j < pMesh->m_vecBones[i].vecKeyFrame.size(); ++j)
            {
                vecFrameTrans[static_cast<UINT>(pMesh->m_vecBones.size()) * j + i]
                    = tFrameTrans{Vec4(pMesh->m_vecBones[i].vecKeyFrame[j].vTranslate, 0.f), Vec4(pMesh->m_vecBones[i].vecKeyFrame[j].vScale, 0.f), pMesh->m_vecBones[i].vecKeyFrame[j].qRot};
            }
        }
...
int CMesh::Load(const wstring& _strFilePath)
{
    ...
    if (m_vecAnimClip.size() > 0 && m_vecBones.size() > 0)
    {
        ...
        for (size_t i = 0; i < m_vecBones.size(); ++i)
        {
            ...
            for (size_t j = 0; j < m_vecBones[i].vecKeyFrame.size(); ++j)
            {
                vecFrameTrans[static_cast<UINT>(m_vecBones.size()) * j + i]
                    = tFrameTrans{Vec4(m_vecBones[i].vecKeyFrame[j].vTranslate, 0.f), Vec4(m_vecBones[i].vecKeyFrame[j].vScale, 0.f), Vec4(m_vecBones[i].vecKeyFrame[j].qRot)};
            }
        }
        ...
        m_pBoneFrameData->Create(sizeof(tFrameTrans), static_cast<UINT>(vecOffset.size()) * _iFrameCount
                               , SB_TYPE::READ_ONLY, false, vecFrameTrans.data());
    }
