...
int ParticleSystemUI::render_update()
{
    ...
    // �� ��� OnOff ���, On ���� �ش� ��� ������ �������� ��.
    ModuleOnOff("Spawn Module", "##Spawn_Module", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::PARTICLE_SPAWN)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::PARTICLE_SPAWN)])
        SpawnModule();

    ImGui::NewLine();
    ImGui::Separator();

    ModuleOnOff("Scale Change", "##Scale_Change", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::SCALE_CHANGE)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::SCALE_CHANGE)])
        ScaleModule();

    ImGui::NewLine();
    ImGui::Separator();

    ModuleOnOff("Color Change", "##Color_Change", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::COLOR_CHANGE)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::COLOR_CHANGE)])
        ColorModule();
    ...
    ModuleOnOff("Velocity", "##Velocity_Module", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ADD_VELOCITY)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ADD_VELOCITY)])
        AddVelocityModule();

    ModuleOnOff("Drag        ", "##Drag_Module", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::DRAG)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::DRAG)])
        DragModule();

    ImGui::NewLine();
    ImGui::Separator();

    ModuleOnOff("Random Force", "##Random_Force", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::NOISE_FORCE)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::NOISE_FORCE)])
        RandomForceModule();

    ImGui::NewLine();
    ImGui::Separator();

    ModuleOnOff("Render", "##Render_Module", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::RENDER)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::RENDER)])
        RenderModule();

    ImGui::NewLine();
    ImGui::Separator();

    ModuleOnOff("Animation    ", "##Particle_Animation", m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ANIMATION)]);
    if (1 == m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ANIMATION)])
        AnimationModule();
...
void ParticleSystemUI::ModuleOnOff(string _strName, string _strID, int& _ModuleCheck) const
{
    ...
    if (ImGui::ColorButton(szbuffer, Color, ImGuiColorEditFlags_NoTooltip, ImVec2(15.f, 15.f)))
...
void ParticleSystemUI::SetTexture()
{
    ...
    if (ImGui::Button("##TextureSelectBtn", ImVec2(18.f, 18.f)))
    {
        ...
        pListUI->AddDynamic_Select(this, static_cast<UI_DELEGATE_1>(&ParticleSystemUI::SelectTexture));
    }
    //========
    Ptr<CTexture> pTex       = GetTarget()->ParticleSystem()->GetTex();
    char          szbuff[50] = {};
...
void ParticleSystemUI::ColorModule()
{
    ...
    if (ImGui::BeginPopup("mypicker##2"))
    {
        ...
        for (int n = 0; n < IM_ARRAYSIZE(saved_palette1); n++)
        {
            ...
            {
                if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F))
                    memcpy(&saved_palette1[n], payload->Data, sizeof(float) * 3);
                if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F))
                    memcpy(&saved_palette1[n], payload->Data, sizeof(float) * 4);
                ImGui::EndDragDropTarget();
            }
...
void ParticleSystemUI::VelocityScaleModule() const
{
}
...
void ParticleSystemUI::SaveParticle()
{
    ...
    if (ImGui::Button("Save##Particle", ImVec2(100, 18)))
    {
        ...
        {
            wchar_t szStr[256] = {};
            wsprintf(szStr, L"Particle Save ����, Error Number : %d", iErrNum);
            MessageBox(nullptr, szStr, L"��ƼŬ ���� ����", MB_OK);
            return;
        }

        wchar_t szNum[50] = {};
        size_t  iFrmCount = m_tModuleData.AddVelocityType;
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        ...
        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ADD_VELOCITY)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=ADDVELOCITY]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::COLOR_CHANGE)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=COLOR_CHANGE]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::DRAG)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=DRAG]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::NOISE_FORCE)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=NOISE_FORCE]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::PARTICLE_SPAWN)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=PARTICLE_SPAWN]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::RENDER)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=RENDER]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ANIMATION)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
        fwprintf_s(pFile, L"[ModuleCheck=ANIMATION]\n");
        fwprintf_s(pFile, szNum);
        fwprintf_s(pFile, L"\n\n");

        iFrmCount = m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::SCALE_CHANGE)];
        _itow_s(static_cast<int>(iFrmCount), szNum, 50, 10); //������ ���ڿ��� ��ȯ
...
void ParticleSystemUI::LoadParticle()
{
    ...
    if (ImGui::Button("Load##Particle", ImVec2(100, 18)))
    {
        ...
        {
            wchar_t szStr[256] = {};
            wsprintf(szStr, L"Particle Load ����, Error Number : %d", iErrNum);
            MessageBox(nullptr, szStr, L"��ƼŬ �ҷ����� ����", MB_OK);
            return;
        }
        ...
        while (true)
        {
            ...
            fwscanf_s(pFile, L"%s", szBuffer, 256); //���ڿ� ������ ����� ���ڿ��� �а� �� �ǵ�
            ...
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::PARTICLE_SPAWN)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=COLOR_CHANGE]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::COLOR_CHANGE)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=SCALE_CHANGE]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::SCALE_CHANGE)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=ADDVELOCITY]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ADD_VELOCITY)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=DRAG]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::DRAG)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=NOISE_FORCE]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::NOISE_FORCE)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=RENDER]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::RENDER)]);
            }

            else if (!wcscmp(szBuffer, L"[ModuleCheck=ANIMATION]"))
            {
                fwscanf_s(pFile, L"%d", &m_tModuleData.ModuleCheck[static_cast<UINT>(PARTICLE_MODULE::ANIMATION)]);
            }
